...
model = AutoModelForCausalLM.from_pretrained(
    "/Users/user/GenAIBootcamp/FinalProject/DataEnginnering/models/mistral",  # chemin vers le dossier du modèle
    model_file="mistral-7b-instruct-v0.2.Q4_K_M.gguf",  # nom exact du fichier téléchargé
    model_type="mistral",
    gpu_layers=50  # si tu veux utiliser l'accélération Metal GPU
)
...
def extract_personality_traits_batch(batch_personalities):
    ...
    for personalities in batch_personalities:
        prompt = (
            ...
            f"List: {json.dumps(personalities)}"
        )
        output = ask(prompt)
        try:
